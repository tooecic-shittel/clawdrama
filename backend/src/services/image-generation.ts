...
function buildStoryboardIdentityClause(storyboardId: number): string {
  try {
    ...
    return `本画面出现的角色及其身份（必须与参考图严格对应，不得混淆性别、外貌或张冠李戴；必须保持参考图中的发型、发色、服装款式、服装配色、身材比例和标志性道具，除非剧情动作要求，只改变姿势、表情、光照和环境湿度）：${parts.join('；')}。参考图优先级：角色外貌与服装一律以角色立绘参考图（最先出现的人物图）为唯一标准；旧镜头画面仅供构图、光线与场景衔接参考，不得继承其中人物的过时形象或瞬时表情（如眨眼、半闭眼、张嘴）。`
  } catch {
    return ''
  }
}
